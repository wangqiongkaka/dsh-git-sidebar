...
- 变更列表：默认按目录树展示（逐级嵌套，只含一个子目录的目录合并为一行，可折叠，折叠状态与视图选择在切换标签后保留），可从标题栏的「列表 / 树形」或更多操作切换为扁平列表（文件名后显示所在目录）；勾选即 Add，取消勾选即取消 Add，标题栏「全部 Add / 全部取消 Add」一键批量操作；操作后原地刷新，不显示加载占位；状态字母按类型着色；悬停行可查看差异或放弃该文件的更改；右键菜单可「打开编辑器」（走 DSH 文件预览器；`.html` / `.htm` 默认由 DSH 的 HTML 预览器渲染，工具栏的「打开方式」可切回源码）。
