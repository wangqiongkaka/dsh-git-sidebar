...
- 变更列表：默认按目录树展示（逐级嵌套，只含一个子目录的目录合并为一行，可折叠，折叠状态与视图选择在切换标签后保留），可从标题栏的「列表 / 树形」或更多操作切换为扁平列表（文件名后显示所在目录）；勾选即 Add，取消勾选即取消 Add，标题栏「全部 Add / 全部取消 Add」一键批量操作；操作后原地刷新，不显示加载占位；状态字母按类型着色；悬停行可查看差异或放弃该文件的更改；右键菜单可「打开编辑器」（走 DSH 文件预览器），`.html` / `.htm` 文件另有一项「在浏览器中打开」。
...
- 更多操作菜单：按远端 / 分支 / 工作区分组，收纳 Fetch All、推送、合并、变基、Worktree、回到远端状态、全部 Add、存入 Stash、提交为 WIP、撤销 WIP、新建 Tag、放弃所有更改和刷新。
...
差异按会话、文件、提交或提交区间分别打开标签页，布局与分屏由 DSH 管理；文件通过 DSH 文件预览器打开，变更行的右键菜单对 `.html` / `.htm` 文件提供「在浏览器中打开」，由宿主用系统默认浏览器打开该文件（DSH 的 `/api/file` 路由对 HTML 带沙箱 CSP，脚本与外链样式加载不了，设计稿类页面在其中渲染不完整）。远端操作使用本机已有的 Git 认证和身份。
...
「在浏览器中打开」走同一 `/git-sidebar/api/*` 路由的 `fs.open-in-browser`：只接受 `.html` / `.htm` 后缀且已存在的普通文件，路径经绝对化与参数校验后作为单个 argv 交给 DSH 的 `openNativePath`（无 shell；macOS 优先按 https 关联解析默认浏览器、没有该记录时回退 `.html` 文件关联，Linux 用 `$BROWSER` 后回退 `xdg-open`，Windows 走 `Invoke-Item`），宿主机没有桌面环境时直接拒绝而不会空跑。该能力由插件打包时内联，不新增运行时依赖。

## 验证

`pnpm typecheck`、`pnpm test`（53 项）和 `pnpm build` 已通过；`tsc`、Vitest 与打包均在本机 `deepseek-harness` `0.1.6-alpha.2` checkout（开发 `link:` 的目标）上运行。0.1.3 时已在本机 DSH web profile 中检查引导页入口、变更列表与勾选 Add、历史图谱、两组菜单和自动刷新；之后新增的列表 / 树切换与目录折叠、WIP 提交、回到远端状态、同步变基、历史自动加载、引导页卡片和 Worktree 会话导航由单元测试覆盖，未在本机 DSH 中逐项实测。未执行用户仓库的推送或丢弃操作。「添加到聊天」「解释更改」依赖 DSH 会话接口，未实测发送。「在浏览器中打开」由路由测试注入的 opener 验证（只接受现存 `.html` / `.htm`、其余输入在被调用前返回 400、启动失败返回 500 并带宿主消息）与视图测试验证（菜单项仅对 HTML 行出现、失败消息显示在面板里）；真实浏览器启动与安装包在本机 DSH 中的实测未执行。
